...
// script.js — robust paste with modal/clipboard/prompt fallbacks
(function(){
'use strict';
...
ready(function(){
  ...
  const I18N = {
    en: {
      ...
      promptTitle: "Paste here (one or two columns, TAB/comma/space separated):"
    },
    zh: {
      title: "时间码批量计算器",
      ...
      promptTitle: "把内容粘在这里（支持一列或两列，TAB/逗号/空格分隔）："
    }
  };
  ...
  function applyI18n(lang){
    ...
    document.title = dict.title + " · Portal";
    document.querySelectorAll('[data-i18n]').forEach(el=>{
      const key = el.getAttribute('data-i18n'); if (dict[key]) el.textContent = dict[key];
    });
    const legend = document.getElementById('legendHTML'); if (legend) legend.innerHTML = dict.legendHTML;
    ...
    const pasteBox = document.getElementById('pasteBox'); if (pasteBox) pasteBox.placeholder = dict.pastePlaceholder;
    const sample = document.getElementById('pasteSample'); if (sample) sample.innerHTML = dict.pasteSample;
    const customFps = document.getElementById('customFps'); if (customFps) customFps.placeholder = (lang === 'zh') ? "例如 47.952" : "e.g. 47.952";
  }
  ...
  function getFpsProfile(){
    ...
    return FPS_PROFILES[preset] || FPS_PROFILES["29.97"];
  }
  function setDropFrameAvailability(){
    const df=document.getElementById('dropFrame'); if(!df) return;
    const {allowDF}=getFpsProfile();
    df.disabled=!allowDF; if(!allowDF) df.checked=false;
  }
  function autoColonize(s){
    const neg=s.startsWith('-'); const digits=s.replace(/[^0-9]/g,'');
    if(digits.length===8){
  ...
  function parseTCToParts(tc){
    let s=tc.trim(); if(s==='') return null; const neg=s.startsWith('-'); if(neg) s=s.slice(1);
    if(/^\d{8}$/.test(s)) s=autoColonize(s); s=s.replace(/;/g,':');
    const m=s.match(/^(\d{1,2}):([0-5]?\d):([0-5]?\d):(\d{1,2})$/);
    if(!m) return null; const [_,HH,MM,SS,FF]=m; return {neg,h:+HH,m:+MM,s:+SS,f:+FF};
  }
  function fmtTC(h,m,s,f,useSemicolon=false,timebase=30){
    ...
    if(f>=timebase) f=timebase-1; const FF=String(f).padStart(2,'0');
    const sep = useSemicolon?';':':'; return `${HH}:${MM}:${SS}${sep}${FF}`;
  }
  function ndfToFrames(parts,timebase){ const {h,m,s,f}=parts; return ((h*3600+m*60+s)*timebase)+f; }
  function framesToNDF(frames,timebase){
    const sign=frames<0?-1:1; let fr=Math.abs(frames);
    const hourFrames=timebase*3600; const minuteFrames=timebase*60;
  ...
  function dfToFrames(parts,fpsExact){
    const timebase=Math.round(fpsExact); const drop=dropFramesCount(fpsExact);
    const hourFrames=timebase*3600; const minuteFrames=timebase*60;
  ...
  function framesToDF(frames,fpsExact){
    const sign=frames<0?-1:1; let fr=Math.abs(frames);
    const timebase=Math.round(fpsExact); const drop=dropFramesCount(fpsExact);
    const framesPerHour=timebase*3600; const framesPer24Hours=framesPerHour*24;
    const framesPer10Minutes=timebase*60*10; const framesPerMinute=(timebase*60)-drop;
    fr=fr%framesPer24Hours;
    const d=Math.floor(fr/framesPer10Minutes);
    let m=fr%framesPer10Minutes;
    if(m>=drop){ fr=fr+(drop*9*d)+drop*Math.floor((m-drop)/framesPerMinute); }
    else{ fr=fr+(drop*9*d); }
    const tb=timebase;
    const f=fr%tb; const s=Math.floor(fr/tb)%60;
  ...
  function makeRow(){
    ...
    const a=row.querySelector('.a'), b=row.querySelector('.b');
    a.addEventListener('input',e=>{const v=e.target.value; const c=autoColonize(v); if(c!==v) e.target.value=c;});
    b.addEventListener('input',e=>{const v=e.target.value; const c=autoColonize(v); if(c!==v) e.target.value=c;});
    grid.appendChild(row);
  }
  function addRows(n){ for(let i=0;i<n;i++) makeRow(); }
  function resetGrid(){ grid.innerHTML=''; addRows(20); applyI18n(getLang()); }

  if (grid && grid.children.length === 0) addRows(20);

  const addRowsBtn=document.getElementById('addRows'); if(addRowsBtn) addRowsBtn.onclick=()=>addRows(10);
  const clearRowsBtn=document.getElementById('clearRows'); if(clearRowsBtn) clearRowsBtn.onclick=resetGrid;

  // ===== calculation =====
  function calculate(){
    try{
      const prof=getFpsProfile(); const exact=prof.exact; const timebase=prof.timebase;
      const useDF=!!(document.getElementById('dropFrame')&&document.getElementById('dropFrame').checked);
      const opSel=document.getElementById('op'); const outSel=document.getElementById('outMode');
      const op=opSel?opSel.value:'diff'; const outMode=outSel?outSel.value:'both';
      [...grid.children].forEach((row)=>{
        ...
        aIn.classList.remove('bad'); bIn.classList.remove('bad'); resTC.textContent=''; resFrames.textContent='';
        ...
        const check=(p)=>p && p.f<timebase;
        let ok=true;
        if(aParts && !check(aParts)){ aIn.classList.add('bad'); ok=false; }
        if(bParts && !check(bParts)){ bIn.classList.add('bad'); ok=false; }
        if(!ok) return;
        ...
        const rTC=fromFrames(rF);
        const semicolon=!!useDF;
        const tcText=(rTC.sign<0?'-':'')+fmtTC(rTC.h,rTC.m,rTC.s,rTC.f,semicolon,timebase);
        const framesText=(rTC.sign<0?'-':'')+Math.abs(rF).toString();
    ...
    }catch(err){ console.error('Calculate error:', err); alert('JS error: '+(err?.message||err)); }
  }
  const calcBtn=document.getElementById('calcBtn'); if(calcBtn) calcBtn.addEventListener('click', calculate);
  document.addEventListener('keydown',(e)=>{ if(e.key==='Enter'&&(e.metaKey||e.ctrlKey)) calculate(); });

  // fps preset toggle for custom
  const fpsSel=document.getElementById('fpsPreset');
  if (fpsSel){
    fpsSel.addEventListener('change', ()=>{
      const wrap=document.getElementById('customFpsWrap');
      if(wrap) wrap.classList.toggle('hidden', fpsSel.value!=='custom');
      setDropFrameAvailability();
    });
  }
  const customFpsInp=document.getElementById('customFps'); if (customFpsInp) customFpsInp.addEventListener('input', setDropFrameAvailability);
  setDropFrameAvailability();

  // ===== paste dialog + fallbacks =====
  let pasteMode='A';
  const pasteModal=document.getElementById('pasteModal');
  const pasteBox=document.getElementById('pasteBox');
  const pasteTitle=document.getElementById('pasteTitle');

  function parseAndApply(text, mode){
    ...
    if(mode==='PAIR'){
      ...
      for(const line of lines){
        if(grid.children[r]==null) addRows(10);
        grid.children[r].querySelector(mode==='A'?'.a':'.b').value=autoColonize(line);
        r++;
      }
    }
    calculate();
  ...
  async function quickPaste(mode){
    const dict=I18N[getLang()]||I18N.en;
    try{
      if(navigator.clipboard && navigator.clipboard.readText){
        const text = await navigator.clipboard.readText();
        if(text && text.trim()){ parseAndApply(text, mode); return; }
      }
    }catch(e){ /* ignore, fallback to prompt */ }
    const fallback = window.prompt(dict.promptTitle, "");
    if (fallback != null) parseAndApply(fallback, mode);
  }

  function openPasteUI(mode){
    if(pasteModal && typeof pasteModal.showModal === 'function'){
      pasteMode=mode;
      const lang=getLang();
      if(pasteTitle){
        pasteTitle.textContent = mode==='PAIR'
          ? (lang==='zh'?'批量粘贴（两列：A + B）':'Bulk Paste (two columns: A + B)')
          : (lang==='zh'?`批量粘贴到列 ${mode}`:`Bulk Paste to column ${mode}`);
      }
      if(pasteBox) pasteBox.value='';
      pasteModal.showModal();
    }else{
      // No <dialog> support or missing dialog — fallback to clipboard/prompt
      quickPaste(mode);
    }
  }

  document.querySelectorAll('[data-paste]').forEach(btn=>{
    btn.addEventListener('click', ()=> openPasteUI(btn.dataset.paste));
  });

  const pasteConfirm=document.getElementById('pasteConfirm');
  if (pasteConfirm){
    pasteConfirm.addEventListener('click', (e)=>{
      e.preventDefault();
      const text=(pasteBox?.value||'').trim();
      if(!text){ pasteModal?.close?.(); return; }
      parseAndApply(text, pasteMode);
      pasteModal?.close?.();
    });
  }
  ...
  function copyColumn(which){
    const vals=[...grid.children].map(row=>{
      return which==='tc'
        ? row.querySelector('.res-tc').textContent.trim()
  ...
  const copyTC=document.getElementById('copyTC'); if(copyTC) copyTC.addEventListener('click', ()=>copyColumn('tc'));
  const copyFrames=document.getElementById('copyFrames'); if(copyFrames) copyFrames.addEventListener('click', ()=>copyColumn('frames'));

  // ===== language toggle =====
  const langBtn=document.getElementById('langToggle'); if(langBtn) langBtn.addEventListener('click', ()=>toggleLang());
  applyI18n(getLang());
